...
inline bool TooManyFailures(
    const std::vector<libice::Connection::SentPing>& pings_since_last_response,
...
webrtc::IceCandidateType GetCandidateTypeByString(const std::string& type) {
  if (type == libice::LOCAL_PORT_TYPE) {
    return webrtc::IceCandidateType::kLocal;
  } else if (type == libice::STUN_PORT_TYPE) {
    return webrtc::IceCandidateType::kStun;
  } else if (type == libice::PRFLX_PORT_TYPE) {
    return webrtc::IceCandidateType::kPrflx;
  } else if (type == libice::RELAY_PORT_TYPE) {
...
const libice::IceFieldTrials kDefaultFieldTrials;

constexpr int kSupportGoogPingVersionRequestIndex =
   static_cast<int>(libice::IceGoogMiscInfoBindingRequestAttributeIndex::
                         SUPPORT_GOOG_PING_VERSION);

constexpr int kSupportGoogPingVersionResponseIndex = 
    static_cast<int>(libice::IceGoogMiscInfoBindingResponseAttributeIndex::
                         SUPPORT_GOOG_PING_VERSION);
...
void ConnectionRequest::Prepare(StunMessage* request) {
 ...
 /* if (webrtc::field_trial::IsEnabled(
  ...
  uint32_t type_preference =
      (connection_->local_candidate().protocol() == cricket::TCP_PROTOCOL_NAME)
