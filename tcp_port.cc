...
Connection* TCPPort::CreateConnection(const Candidate& address,
  ...
  if ((address.protocol() == cricket::SSLTCP_PROTOCOL_NAME) &&
  ...
  if (listen_socket_) {
    ...
    if (listen_socket_->GetState() == rtc::AsyncPacketSocket::STATE_BOUND ||
      ...
      AddAddress(listen_socket_->GetLocalAddress(),
		  ...
		  cricket::TCP_PROTOCOL_NAME, "", TCPTYPE_PASSIVE_STR, LOCAL_PORT_TYPE,
    ...
    AddAddress(rtc::SocketAddress(Network()->GetBestIP(), DISCARD_PORT),
               ...
               rtc::SocketAddress(), cricket::TCP_PROTOCOL_NAME, "", TCPTYPE_ACTIVE_STR,
...
bool TCPPort::SupportsProtocol(const std::string& protocol) const {
  return protocol == cricket::TCP_PROTOCOL_NAME || protocol == cricket::SSLTCP_PROTOCOL_NAME;
}
...
TCPConnection::TCPConnection(TCPPort* port,
    ...
    : Connection(port, 0, candidate),
      ...
      reconnection_timeout_(libice::CONNECTION_WRITE_CONNECT_TIMEOUT) {
  if (outgoing_) {
...
void TCPConnection::CreateOutgoingTcpSocket() {
  ...
  int opts = (remote_candidate().protocol() == cricket::SSLTCP_PROTOCOL_NAME)
