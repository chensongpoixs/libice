...
bool IsRelayRelay(const libice::Connection* conn) {
  return conn->local_candidate().type() == libice::RELAY_PORT_TYPE &&
         conn->remote_candidate().type() == libice::RELAY_PORT_TYPE;
}

bool IsUdp(const libice::Connection* conn) {
...
int CompareCandidatePairsByNetworkPreference(
    const libice::Connection* a,
    const libice::Connection* b,
    absl::optional<rtc::AdapterType> network_preference) {
