...
int GetProtocolPriority(libice::ProtocolType protocol) {
  switch (protocol) {
    case libice::PROTO_UDP:
      return 2;
    case libice::PROTO_TCP:
      return 1;
    case libice::PROTO_SSLTCP:
    case libice::PROTO_TLS:
      return 0;
    default:
      RTC_NOTREACHED();
      return 0;
...
int ComparePort(const libice::Port* a, const libice::Port* b) {
...
void BasicPortAllocatorSession::SetCandidateFilter(uint32_t filter) {
  ...
  for (PortData& port_data : ports_) {
    ...
    libice::Port* port = port_data.port();
...
void BasicPortAllocatorSession::StartGettingPorts() {
  ...
  if (!socket_factory_) {
    owned_socket_factory_.reset(
        new libice::BasicPacketSocketFactory(network_thread_->socketserver()));
...
bool BasicPortAllocatorSession::CandidatePairable(const Candidate& c,
  ...
  bool can_ping_from_candidate =
      (port->SharedSocket() || c.protocol() == cricket::TCP_PROTOCOL_NAME);
