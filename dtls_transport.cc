...
//       impl again
void DtlsTransport::OnWritableState(libice::PacketTransportInternal* transport) {
...
void DtlsTransport::OnReceivingState(libice::PacketTransportInternal* transport) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  RTC_DCHECK(transport == ice_transport_);
  RTC_LOG(LS_VERBOSE) << ToString()
                      << ": ice_transport "
...
void DtlsTransport::OnReadPacket(libice::PacketTransportInternal* transport,
                                 const char* data,
                                 size_t size,
...
void DtlsTransport::OnSentPacket(libice::PacketTransportInternal* transport,
                                 const rtc::SentPacket& sent_packet) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  SignalSentPacket(this, sent_packet);
}

void DtlsTransport::OnReadyToSend(libice::PacketTransportInternal* transport) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  if (writable()) {
