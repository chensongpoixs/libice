...
rtc::AsyncPacketSocket* BasicPacketSocketFactory::CreateUdpSocket(
    const rtc::SocketAddress& address,
    uint16_t min_port,
    uint16_t max_port) {
  // UDP sockets are simple.
	rtc::Socket* socket = socket_factory_->CreateSocket(address.family(), SOCK_DGRAM);
  if (!socket) {
    return NULL;
  }
...
rtc::AsyncListenSocket* BasicPacketSocketFactory::CreateServerTcpSocket(
...
rtc::AsyncPacketSocket* BasicPacketSocketFactory::CreateClientTcpSocket(
    const rtc::SocketAddress& local_address,
  ...
  if (tcp_options.opts & PacketSocketFactory::OPT_STUN) {
    tcp_socket = new libice::AsyncStunTCPSocket(socket);
  } else {
...
rtc::AsyncResolverInterface* BasicPacketSocketFactory::CreateAsyncResolver() {
  return new rtc::AsyncResolver();
}

int BasicPacketSocketFactory::BindSocket(rtc::Socket* socket,
                                         const rtc::SocketAddress& local_address,
                                         uint16_t min_port,
                                         uint16_t max_port) {
  int ret = -1;
