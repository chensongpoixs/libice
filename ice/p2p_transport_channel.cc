...
libice::PortInterface::CandidateOrigin GetOrigin(
	libice::PortInterface* port,
	libice::PortInterface* origin_port) {
  if (!origin_port)
    return libice::PortInterface::ORIGIN_MESSAGE;
  else if (port == origin_port)
    return libice::PortInterface::ORIGIN_THIS_PORT;
  else
    return libice::PortInterface::ORIGIN_OTHER_PORT;
}
...
uint32_t GetWeakPingIntervalInFieldTrial() {
  ...
  return libice::WEAK_PING_INTERVAL;
}
...
P2PTransportChannel::P2PTransportChannel(
  ...
  libice::BasicRegatheringController::Config regathering_config;
  ...
  regathering_controller_ =
      std::make_unique<libice::BasicRegatheringController>(
...
P2PTransportChannel::P2PTransportChannel(
    ...
    : P2PTransportChannel(
          ...
          allocator,
          nullptr,
          std::make_unique<libice::WrappingAsyncDnsResolverFactory>(
...
IceTransportState P2PTransportChannel::ComputeState() const {
  RTC_DCHECK_RUN_ON(network_thread_);
  if (!had_connection_) {
    return libice::IceTransportState::STATE_INIT;
  }
  ...
  if (active_connections.empty()) {
    return libice::IceTransportState::STATE_FAILED;
  }
...
void P2PTransportChannel::SwitchSelectedConnection(Connection* conn,
  ...
  if (selected_connection_) {
    ...
    network_route_->packet_overhead =
		...
		cricket::GetProtocolOverhead(selected_connection_->local_candidate().protocol());
  } else {
