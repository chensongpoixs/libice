...
#ifndef _C_P2P_BASE_REGATHERING_CONTROLLER_H_
#define _C_P2P_BASE_REGATHERING_CONTROLLER_H_
...
class BasicRegatheringController : public sigslot::has_slots<> {
 public:
  struct Config {
    int regather_on_failed_networks_interval =
        libice::REGATHER_ON_FAILED_NETWORKS_INTERVAL;
  };

  BasicRegatheringController() = delete;
  BasicRegatheringController(const Config& config,
                             libice::IceTransportInternal* ice_transport,
                             rtc::Thread* thread);
  ...
  // of regathering.
  void Start();
  void set_allocator_session(libice::PortAllocatorSession* allocator_session) {
 ...
 private:
  ...
  void OnIceTransportStateChanged(libice::IceTransportInternal*) {}
  void OnIceTransportWritableState(libice::PacketTransportInternal*) {}
  void OnIceTransportReceivingState(libice::PacketTransportInternal*) {}
  ...
  std::unique_ptr<webrtc::ScopedTaskSafety> pending_regathering_;
  Config config_;
  libice::IceTransportInternal* ice_transport_;
  libice::PortAllocatorSession* allocator_session_ = nullptr;
